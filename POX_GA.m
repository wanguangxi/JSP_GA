...
mpopulation = unique(population,'row');
initial_similarity = size(mpopulation,1)/size(population,1);
...
for iterator = 1:Iterations
    ...
    similarity = 0;
    dynanmic_factor = ((iterator-Iterations)/Iterations)*5;
    threshold = 0.8*initial_similarity-1/(1+exp(-dynanmic_factor));
    if threshold <=0
        threshold = 0;
    end
    ...
    %         if memeber==(PopSize-1)/2
    %             memeber=1;
    %         end
      %  disp(memeber)
        end 
        unique_parent = unique(Parent,'rows');
        similarity = size(unique_parent,1)/size(Parent,1);  
   ...
   %% POX Crossover 
    ...
    for i=1:(PopSize-1)/2
        ...
        if rand(1)<=Pc
            %随机划分工件集{1,2,3，...,n}为两个非空子集J1和J2。
            num_J1 = randi([1,num_of_jobs]);   %随机确定两个子集的大小
            if num_J1==num_of_jobs
                num_J1 = fix(num_of_jobs/2);  
            end
            J = randperm(num_of_jobs);  %所有的工件的随机序列        
            J1 =J(1:num_J1);
           % J2 =J(num_J1+1:n); 
            %复制Parent1包含在J1中的工件到Children1,Parent2包含在J1中的工件到Children2，保留它们的位置。
            for index = 1:num_J1
                %寻找Parent1和Parent2包含在J1中的工件
                job = J1(index);
            ...
            %复制Parent1包含在J2中的工件到Children2,Parent2包含在J2中的工件到Children1，保留它们的顺序。
